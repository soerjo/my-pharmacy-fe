...
import type { DispenseOrderFormValues, DispenseOrder } from "@/features/dispense-order/types";
...
export function DispenseOrdersProvider({ children }: { children: ReactNode }) {
  ...
  const ordersQuery = useQuery({
    ...
    queryFn: () => apiClient.get<ApiResponse<PaginatedResponse<DispenseOrder>>>(API_ROUTES.dispenseOrders),
    select: (response) => response.data,
  });

  const createMutation = useMutation({
    mutationFn: (data: DispenseOrderFormValues) =>
      apiClient.post<ApiResponse<DispenseOrder>>(API_ROUTES.dispenseOrders, data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: DispenseOrderFormValues }) =>
      apiClient.put<ApiResponse<DispenseOrder>>(`${API_ROUTES.dispenseOrders}/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.dispenseOrders.all });
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient.delete<ApiResponse<void>>(`${API_ROUTES.dispenseOrders}/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.dispenseOrders.all });
    },
  });
